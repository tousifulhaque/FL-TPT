...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None):
        ...
        src = self.norm1(src)
        src2 = self.linear2(self.tanh(self.linear1(src)))
        src = src + self.dropout(src2)
        src = self.norm2(src)
        return src

class Transformer(nn.Module):
    def __init__(self,input_size = 64,output_size = 5, num_layers = 4, hidden_size=64, n_heads= 4, dim_feedforward=128, dropout=0.1):
        super(Transformer, self).__init__()
        self.embed = nn.Linear(input_size, hidden_size)
        self.layers = nn.ModuleList([TransformerEncoderLayer(hidden_size, n_heads, dim_feedforward, dropout) for _ in range(num_layers)])
