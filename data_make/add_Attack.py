...
def load_data(csv_file,add_count):
    dataS = pd.read_csv(csv_file, header=['vehicle_ID','attackerType','time'	, 'pos_x'	, 'pos_y'	
                                          ,'spd_x',	'spd_y', 'disChange_X' , 'disChange_Y'	,'spdChange_X',
                                            'spdChange_Y','RSSI'])
    ...
    for vid in dataS.vehicle_ID.unique():
        ...
        if headerFlag==0:
            frame_ori.to_csv("add_moreAttack.csv",mode='a',ignore_index = True,header=True)
            headerFlag=1
        else:
