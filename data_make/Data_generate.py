...
def walkFile(file):
    ...
    for root, dirs, files in tqdm(os.walk(file), unit = "dirs"):
        # root 表示当前正在访问的文件夹路径
    ...
    # result_df = pd.concat(list_df, ignore_index=False)
    # result_df.to_csv('./data/result.csv',mode='a', index=True,header=list_df[0].columns)
        # 遍历所有的文件夹
        # for d in dirs:
...
def readJson(filePath,count,groundTruthFilePath,HeaderFlag):
    ...
    if HeaderFlag==0:
        df.to_csv('./data/result.csv',mode='a', index=False,header=True)
    else:
        df.to_csv('./data/result.csv',mode='a', index=False,header=False)
    print(filePath)
    #return df
